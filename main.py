...
async def chat_endpoint(message: str):
    print(float("nan"))
    response = {"message": await chat_main(message)}
...
async def queryV3(symbol: str):
    ...
    try:
        if symbol.split(".")[1] == "NS":
            ...
            resultNSE = dfNSE.loc[
                dfNSE["SYMBOL"] == symbol.split(".")[0],
                ["COMPANY", "ISIN", "FACE_VALUE"],
            ]
            ...
            if symbol.split(".")[1] == "BO":
                ...
                result = df.loc[
                    ...
                    ["FaceValue", "ISIN", "IndustryNew", "STOCK"],
                ]
                stockIndustry = result["IndustryNew"].values[0]
...
async def info(symbol: str):
    ...
    JSON = {"symbol": symbol, "stock_name": stock_name, "quote_type": quote_type}
...
async def prediction(symbol: str):
    prediction = fetchStockPrediction(symbol)
    jsonData = {"predicted_close": prediction}
    return jsonData
...
def hourlyPrediction(symbol: str):
    ...
    jsonData = {"predicted_close": float(prediction[0][0])}
    return jsonData
...
def fetchStockPrediction(symbol: str) -> float:
    """
